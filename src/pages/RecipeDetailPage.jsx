...
const RecipeDetailPage = () => {
    ...
    const getBeautifulFoodImage = () => {
        ...
        if (t.includes('biryani')) keyword = 'biryani';
        else if (t.includes('cake')) keyword = 'cake';
        else if (t.includes('curry')) keyword = 'curry';
        else if (t.includes('shake') || t.includes('smoothie')) keyword = 'milkshake';
        else if (t.includes('pizza')) keyword = 'pizza';
        else if (t.includes('burger')) keyword = 'burger';
        else if (t.includes('pasta')) keyword = 'pasta';
        else if (t.includes('ice cream')) keyword = 'icecream';
        else if (t.includes('rice') || t.includes('pulao')) keyword = 'rice';
        else if (t.includes('chicken') || t.includes('mutton') || t.includes('fish')) keyword = 'meat';
        else if (t.includes('salad')) keyword = 'salad';

        let hash = recipe.id || 0;
        for (let i = 0; i < t.length; i++) {
            hash = t.charCodeAt(i) + ((hash << 5) - hash);
        }
        const lockSeed = Math.abs(hash) % 5000;
        return `https://loremflickr.com/1200/600/food,${keyword}/all?lock=${lockSeed}`;
    };
